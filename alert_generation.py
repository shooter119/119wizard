...
def load_contacts():
    ...
    try:
        ...
        return raw_data, contacts_map
    except Exception as e:
        print(f"❌ 加载联系人数据库失败: {e}")
        return [], {}

CONTACTS_LIST, CONTACTS_MAP = load_contacts()
...
# V2 语义向量库（用于输出安全提示/处置要点/灾害特点/作战要则/特别警示）
_V2_VECTORS = None

def load_vectors_v2():
    global _V2_VECTORS
    if _V2_VECTORS is not None:
        return _V2_VECTORS
    try:
        vector_path = os.path.join(BASE_DIR, 'intelligence base', 'fire_vectors_cleaned_v2_agent.json')
        with open(vector_path, 'r', encoding='utf-8') as f:
            _V2_VECTORS = json.load(f)
    except Exception:
        _V2_VECTORS = []
    return _V2_VECTORS
...
def _phase_rank(phase, section):
    preferred = {
        'safety_tips': ['准备阶段', '风险控制阶段'],
        'handling_points': ['处置阶段'],
        'characteristics': ['态势研判阶段'],
        'tactics': ['作战阶段'],
        'warnings': ['风险控制阶段']
    }
    order = preferred.get(section, [])
    if phase in order:
        return order.index(phase)
    return 9

def build_v2_content(case_id='', case_name='', category='', description=''):
    vectors = load_vectors_v2()
    if not vectors:
        return {}
    ...
    def _match_primary(item):
        if case_id and item.get('case_id') == case_id:
            return True
        if case_name and item.get('case_name') == case_name:
            return True
        return False

    def _match_fallback(item):
        tags = item.get('scenario_tags') or []
        text = item.get('content') or ''
        if case_name and any(case_name in str(t) for t in tags):
            return True
        if category and any(category in str(t) for t in tags):
            return True
        if case_name and case_name in str(text):
            return True
        if description and description in str(text):
            return True
        return False

    matched = [v for v in vectors if _match_primary(v)]
    if not matched:
        matched = [v for v in vectors if _match_fallback(v)]

    if not matched:
        return {}

    type_map = {
        'safety_tip': 'safety_tips',
        'handling_point': 'handling_points',
        'characteristic': 'characteristics',
        'tactic': 'tactics',
        'warning': 'warnings'
    }

    grouped = {k: [] for k in type_map.values()}
    for item in matched:
        section = type_map.get(item.get('knowledge_type'))
        if not section:
            continue
    ...
    for section, items in grouped.items():
        ...
        items.sort(key=lambda x: (
            -_priority_score(x.get('priority')),
            -(x.get('risk_level_estimated') or 0),
            _role_rank(x.get('decision_role')),
            _phase_rank(x.get('tactical_phase'), section)
        ))
        seen = set()
        texts = []
        for item in items:
...
def geocode(address, city="330800", nearby_point=None):
    ...
    if geocode_result:
        ...
        if should_trigger_poi:
            ...
            building_name = (geocode_result.get("building") or {}).get("name", "")
            neighborhood_name = (geocode_result.get("neighborhood") or {}).get("name", "")
            point_name = building_name or neighborhood_name or geocode_result.get("formatted_address", "")
            return {
                "name": point_name,
    ...
    try:
        ...
        if data.get("status") == "1" and data.get("pois"):
            ...
            # 规则3：强关键词约束（单位名/地点名必须命中）
            def _extract_strong_keywords(text):
                if not text:
                    return []
                # 去除常见行政/行业通用词
                stop_words = [
                    "省","市","县","区","镇","乡","街道","村","社区","居委会",
                    "公司","厂","车间","化工","工业","有限","有限责任","集团",
                    "股份","厂房","仓库","中心","企业","实业","园区",
                    "服务区","出口","入口","收费站","互通"
                ]
                cleaned = text
                for w in stop_words:
                ...
                # 提取连续中文片段
                parts = re.findall(r'[\u4e00-\u9fa5]{2,}', cleaned)
                # 去重并按长度排序
                uniq = []
                for p in parts:
                    if p not in uniq:
            ...
            strong_keys = _extract_strong_keywords(address)
            if strong_keys:
                filtered = [p for p in pois if any(k in (p.get("name") or "") for k in strong_keys)]
                if filtered:
                    print(f"🔎 强关键词过滤生效：{strong_keys}，候选 {len(pois)} -> {len(filtered)}")
                    pois = filtered
                else:
                    print(f"⚠️ 强关键词未命中POI名称：{strong_keys}，保持原候选")
            
            # 跨区域增援场景下禁用距离排序，避免误匹配
...
def calculate_force_stats(vehicles, station_name=None):
    ...
    for v_str in vehicles:
        ...
        for db_v in search_pool:
            ...
            db_plate = normalize_str(db_v.get("车牌号", ""))
            ...
            # 车牌号精确/包含匹配（最高优先）
            if db_plate and (v_normalized == db_plate or db_plate in v_normalized):
                score += 20
...
def extract_location_from_address(address):
    ...
    # 提取村/社区名
    village_match = re.search(r'([\u4e00-\u9fa5]+(?:村|社区|居委会))', address)
    if village_match:
        village = village_match.group(1)
    
    return county, street, village

def _is_leader(position):
    return any(role in position for role in ['书记', '主任', '镇长', '所长'])

def _pick_best_contact(contacts):
    if not contacts:
        return None
    leaders = [c for c in contacts if _is_leader(c.get('position', ''))]
    return leaders[0] if leaders else contacts[0]

def find_contacts_by_address(address):
    """根据地址直接匹配联系人（优先村/社区，其次乡镇）"""
    if not address:
        return []
    matches = []
    # 优先按村/社区名称匹配
    for entry in CONTACTS_LIST:
        ...
        if village and village in address:
    ...
    for entry in CONTACTS_LIST:
        ...
        if township and township in address:
            matches.append(entry)
    return matches
...
def generate_alert(case_data):
    ...
    station = case_data.get('station') or '永安路站'
    ...
    # 记录地址解析原始/采信数据，供日志使用
    case_data['_geocode_raw'] = geocode_result
    case_data['_geocode_used'] = {
        "formatted_address": formatted_address,
        "adcode": adcode,
        "location": geocode_result.get('location') if geocode_result else None,
        "name": geocode_result.get('name') if geocode_result else None
    }
    ...
    if geocode_result:
        ...
        if route:
            ...
            # 在地址路况中使用完整的消防站名称
            full_station_name = normalize_station_name(station) or "出动消防站"

            traffic_status = route.get('traffic', '路况未知')
            point_name = geocode_result.get("name") or geocode_result.get("formatted_address", "目的地")
            route_info = f"{full_station_name}至{point_name}距离{distance}公里，约{duration}分钟，{traffic_status}。"
        ...
        # 在地址路况中使用完整的消防站名称
        full_station_name = normalize_station_name(station)
        point_name = geocode_result.get("name") if geocode_result else formatted_address
        route_info = f"{full_station_name}前往{point_name}（目的地尚不明确，请尽快核实）。"
    ...
    # 优先使用单位类型检测结果（但尊重用户明确的具体类型）
    user_override_types = ['草木火灾']
    if unit_detection['detected'] and unit_detection['priority'] in ['critical', 'high'] and user_case_type not in user_override_types:
        ...
        if user_case_type in user_override_types and unit_detection['detected']:
            print(f"   💡 用户明确为 {user_case_type}，忽略单位类型 {unit_detection['fire_type_name']}")
        # 未检测到单位，使用用户输入的类型
    ...
    specific_scenarios = ['车辆火灾', '身体部位被卡', '电梯困人', '高空救援', '车祸救援', '摘马蜂窝', '水域救援', '草木火灾']
    ...
    # === 使用 V2 语义向量库补充/覆盖内容（不改动输出模板） ===
    try:
        v2_content = build_v2_content(
            case_id=case_data_info.get('id', ''),
            case_name=case_data_info.get('name', user_case_type),
            category=case_data_info.get('category', ''),
            description=case_data.get('case_description', '')
        )
        if v2_content:
            case_data_info = case_data_info.copy()
            merged_content = dict(case_data_info.get('content') or {})
            for key, value in v2_content.items():
                if value:
                    merged_content[key] = value
            case_data_info['content'] = merged_content
    except Exception:
        pass

    # 5. 联动信息 - 使用纠正后的地址进行匹配
    contact_info = ""
    corrected_address_for_contacts = address

    # 从纠正后的地址中提取街道和村名
    corrected_county, corrected_street, corrected_village = extract_location_from_address(corrected_address_for_contacts)

    # 优先按地址中的村/社区名称直接匹配联系人
    direct_matches = find_contacts_by_address(corrected_address_for_contacts)
    if direct_matches:
        grouped = {}
        for item in direct_matches:
            key = item.get('village') or item.get('township') or ''
            if not key:
                continue
            grouped.setdefault(key, []).append(item)
        for key, group in grouped.items():
            best = _pick_best_contact(group)
            if best:
                contact_info += f"{key}{best.get('position','')} {best.get('name','')}：{best.get('phone','')}；"
    else:
        # 回退：用纠偏后的街道/村名查询联系人
        if corrected_street and CONTACTS_MAP.get(corrected_street):
            street_contact = CONTACTS_MAP[corrected_street]['primary']
            contact_info += f"{corrected_street}{street_contact['position']} {street_contact['name']}：{street_contact['phone']}；"
        
        if corrected_village and CONTACTS_MAP.get(corrected_village):
            village_contact = CONTACTS_MAP[corrected_village]['primary']
            contact_info += f"{corrected_village}{village_contact['position']} {village_contact['name']}：{village_contact['phone']}；"
